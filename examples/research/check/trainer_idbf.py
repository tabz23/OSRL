...
from osrl.algorithms.bc_gaussian_actor import BC, BCTrainer
...
##changes to do: load BC_safe data using the filter on the dataset. 
##changes loss computation to load a bc safe model and then get the unsafe states.
##make sure filter of safe and unsafe is correct.
class CombinedCBFTrainer:
    def __init__(self, model, train_dataset, val_dataset, dt=0.1, lr=1e-4, device="cpu", train_steps=10000, 
                 ...
                 train_dynamics=True, dynamics_lr=1e-4,num_action_samples_cql=10,temp=0.9,detach=False,action_dim=None,bc_model=None,p=0.0001,num_action_samples_idbf=None):
        ...
        self.w_grad = w_grad
        self.w_CQL=w_CQL
        self.num_action_samples_cql=num_action_samples_cql
        self.num_action_samples_idbf=num_action_samples_idbf
        self.temp=temp
        ...
        config = {
            ...
            "num_action_samples_cql":num_action_samples_cql,
            "num_action_samples_idbf":num_action_samples_idbf,
            "temp":self.temp,
            "detach":self.detach,
        }
    ...
    def compute_loss(self, observations, next_observations, actions, costs, training_bool=None):
        ...
        safe_mask = (costs <= 0).reshape(-1, 1)##not needed but keep incase... note that all datapoints form the batch are safe since we load dataset with bc_mode=safe
        # unsafe_mask = (costs > 0).reshape(-1, 1)
        # print(safe_mask)
        # Calculate CBF values for current states
        B = self.model.forward_cbf(next_observations).reshape(-1, 1)##CHANGED TO NEXT_OBSERVATION
        ...
        loss_unsafe = torch.tensor(0.0, device=self.device)
        avg_unsafe_B = torch.tensor(0.0, device=self.device)
        unsafe_acc = 0.0
        B_unsafe = None
        
        # for i in range(5):
        #     for j in range(self.batch_size):
        #         states_tensor = observations[j].unsqueeze(0).to(self.device)
        #         random_actions_tensor = ( 2*torch.rand(1, self.action_dim) - 1 ).to(self.device)##actions between -1,1

        #         pi, _, log_probs = self.bc_model.actor(states_tensor, random_actions_tensor)##i think will be size 1,1 since appending 1 datapt at a time
                
        #         if torch.exp(log_probs).item()<self.p:##won't matter much but sample from the policy itself not from uniform
        #             next_state = self.model.forward_next_state(states_tensor, random_actions_tensor)
        ...
        for i in range(self.num_action_samples_idbf):##previously was 5. if wandb no entry for this param (num_action_samples_idbf) then it was 5
            # Generate random actions for the entire batch at once
            random_actions_tensor = (2 * torch.rand(self.batch_size, self.action_dim) - 1).to(self.device)
            
            # Compute log probabilities for all actions in the batch
            _, _, log_probs = self.bc_model.actor(observations, random_actions_tensor)
            
            # Create a mask for actions with low probability
            low_prob_mask = torch.exp(log_probs) < self.p
            # print("log_probs.shape",log_probs.shape)#256
            # print(torch.exp(log_probs))
            # print("torch.exp(log_probs).shape",torch.exp(log_probs).shape)#256
            
            # Filter out the states and actions that meet the criteria
            # print("observations.shape",observations.shape)
            filtered_observations = observations[low_prob_mask]
            # print("filtered_observations.shape",filtered_observations.shape)##like 243 when p=0.9/ like 225 when p=0.1 / like 143 when p=0.000001 / like 75 when p=0.0000000000001 / like 38 ish when p=1e-20
            filtered_actions = random_actions_tensor[low_prob_mask]
            
            # Compute next states for the filtered batch
            if len(filtered_observations) > 0:
                next_states = self.model.forward_next_state(filtered_observations, filtered_actions)
                unsafe_states.append(next_states)
                unsafe_count += len(filtered_observations)
        ...
        if (unsafe_states):
            ...
            # print(unsafe_states_tensor.shape)#torch.Size([46843, 8]) when p is 0.5 and sampled actions 50  /  torch.Size([2560, 8]) when p=10 and sample 10 actions
        if (unsafe_states):
            B_unsafe = self.model.forward_cbf(unsafe_states_tensor).reshape(-1, 1) ## Added
            loss_unsafe_vector = self.w_unsafe * F.relu(B_unsafe + self.eps_unsafe) ## Added
            loss_unsafe = loss_unsafe_vector.sum() / unsafe_count ## Added
        # wandb.log({"unsafe_count":unsafe_count})
            
        ##
        
    ##ADDED THIS LOSS BE CAREFUL ##ADDED THIS LOSS BE CAREFUL ##ADDED THIS LOSS BE CAREFUL ##ADDED THIS LOSS BE CAREFUL
        # Compute B values
        B_curr = self.model.forward_cbf(observations).reshape(-1, 1)  # Changed to next_observation
        B_next = self.model.forward_cbf(next_observations).reshape(-1, 1)  # Changed to next_observation
        # Lipschitz continuity loss (smoothness constraint)
        # Adjust this hyperparameter as needed
        loss_lip = self.lambda_lip * torch.mean(torch.abs(B_next - B_curr))  # Squared difference penalty
    ...
    ##ADDED THIS LOSS BE CAREFUL ##ADDED THIS LOSS BE CAREFUL ##ADDED THIS LOSS BE CAREFUL ##ADDED THIS LOSS BE CAREFUL
        # Gradient loss computation
           ...
           ##CHANGED TO NEXT_OBSERVATION##CHANGED TO NEXT_OBSERVATION##CHANGED TO NEXT_OBSERVATION##CHANGED TO NEXT_OBSERVATION
        loss_grad = self.compute_gradient_loss(observations, actions, safe_mask)##CHANGED TO OBSERVATIONS
        if (self.w_CQL>0):
        ...
        avg_unsafe_B = B_unsafe.sum() / (unsafe_count + 1e-8)

        # Compute accuracy metrics
        safe_acc = ((B >= 0) * safe_mask).sum() / (num_safe_elements + 1e-8)
        unsafe_acc = ((B_unsafe < 0)).sum() / (unsafe_count + 1e-8)
    ...
    def compute_CQL_loss(self, observations, next_observations, actions, safe_mask):### ASSUMPTION IMP:i will assume observations are safe in a datapoints if cost is 0 even though cost 0 means next obs is safe.
        ...
        all_random_next_h = []
        for _ in range(self.num_action_samples_cql):
    ...
    def train(self):
        ...
        for step in trange(self.train_steps, desc="Training"):
            ...
            if (step+1) % self.eval_every_n_steps == 0:
                ...
                if (total_eval_loss < lowest_eval_loss) and (val_avg_safe_acc>0.5) and (val_avg_unsafe_acc>0.5): ###FIX THS LATER
                # if (total_eval_loss < lowest_eval_loss) and (val_avg_safe_acc>0.7) and (val_avg_unsafe_acc>0.7):
                    ...
                    # Save the hyperparameters to a JSON file
                    with open(hyperparameters_path, 'w') as f:
                        json.dump(hyperparameters, f, indent=4)###FIX THE BELOW ITS NOT BEST ACC
...
def main(args: BCTrainConfig):
    ...
    env.set_target_cost(args.cost_limit)
     ##CHANGED THIS ##CHANGED THIS ##CHANGED THIS ##CHANGED THIS ##CHANGED THIS ##CHANGED THIS ##CHANGED THIS
     
    process_bc_dataset(data, args.cost_limit, args.gamma, bc_mode="safe")##USES ONLY SAFE TRAJECTORIES IN THE DATA. CHANGED THIS
    ...
    # path="/Users/i.k.tabbara/Documents/python directory/OSRL/logs/CUSTOM_GAUSSIAN_OfflineHopperVelocityGymnasium-v1-cost-10/BC-bc83/BC-bc83"
    path="/Users/i.k.tabbara/Documents/python directory/OSRL/logs/OfflineSwimmerVelocityGymnasium-v1-cost-10/BC-8227/BC-8227"
    
    cfg, model = load_config_and_model(path, True)
    ...
    trainer = CombinedCBFTrainer(
        ...
        dynamics_lr=1e-4,
        w_CQL=args.cql,
        num_action_samples_cql=args.num_action_samples_cql,
        num_action_samples_idbf=args.num_action_samples_idbf,
        temp=args.temp,
        detach=args.detach,
        bc_model=bc_model,
        action_dim=env.action_space.shape[0],
...
And/or make the logsumexp_h term smaller (which means making the CBF values of random next states smaller)
    ...
    # python "/Users/i.k.tabbara/Documents/python directory/OSRL/examples/research/check/trainer_idbf.py" --task OfflineHopperVelocityGymnasium-v1 --device="mps"