...
from osrl.common.net import MLPGaussianActor
...
class BC(nn.Module):
    ...
    def __init__(self,
        ...
        self.action_low = -self.max_action * np.ones(action_dim)
        self.action_high = self.max_action * np.ones(action_dim)
    ...
    def actor_loss(self, observations, actions):
        ...
        pi,pred_actions,log_probs=self.actor(observations,actions)
        ...
        stats_actor = {
            ...
            "loss/log_probs": log_probs.mean().item()
        }
        return loss_actor, stats_actor
    ...
    def act(self, obs,deterministic=True):
        ...
        Args:
            obs: The observation
            deterministic (bool): If True, return the mean action instead of sampling
        '''
        obs = torch.tensor(obs[None, ...], dtype=torch.float32).to(self.device)
        # Get distribution, action, and log prob
        _, act, _ = self.actor(obs, deterministic=deterministic)
